...
private String accessToken;
	...
	public void updateContactTest() {
		...
		Response responsePut = restClient.put(BASE_URL_CONTACTS, CONTACTS_ENDPOINT+"/"+contactId, ContentType.JSON, contact, AuthType.BEARER_TOKEN, null, null);
		Assert.assertEquals(responsePut.statusLine(), "HTTP/1.1 200 OK");
		Assert.assertEquals(responsePut.statusCode(), 200);
		Assert.assertEquals(responsePut.jsonPath().getString("street1"), contact.getStreet1());
		Assert.assertEquals(responsePut.jsonPath().getString("city"), contact.getCity());
		...
		Assert.assertEquals(responseGet.jsonPath().getString("street1"), contact.getStreet1());
		Assert.assertEquals(responseGet.jsonPath().getString("city"), contact.getCity());
